...
def main():
    ...
    if st.sidebar.checkbox('Show default probability'):
        ...
        #st.write(f'Default probability: {score:.2f}')
        if score>64:
            
            st.markdown(f'<span style="color:red"> Default probability: {score:.2f} </span>', unsafe_allow_html=True)
        else:
            st.markdown(f'<span style="color:green">Default probability: {score:.2f} </span>', unsafe_allow_html=True)
        
        st.write('Probability limit is 64, below it grant credit!') 
        ...
        if st.checkbox('Show explanations'):
            ...
            _, ax = plt.subplots()
            #ax.axis("equal")
            ax.barh(contribs.index, contribs, color=(contribs > 0).map({True: 'r',
                                                    False: 'g'}))
            ax.set_title("Features contributions")
            #ax.legend(
                #labels,
                #loc='center left',
                #bbox_to_anchor=(0.7, 0.5),
